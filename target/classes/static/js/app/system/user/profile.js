...
$(function () {
    ...
    $(".default_avatars_list").find("img").each(function () {
        ...
        $this.on("click", function () {
            ...
            $.post(ctx + "sysuser/changeAvatar", {"imgName": target_src}, function (r) {
                if (r.code === 0) {
    ...
    $("#update-profile .btn-save").click(function () {
        ...
        if (flag) {
            $.post(ctx + "sysuser/updateUserProfile", $userProfileForm.serialize(), function (r) {
                if (r.code === 0) {
...
function refreshUserProfile() {
    $.post(ctx + "sysuser/profile", function (r) {
        $main_content.html("").append(r);
    });
}

function editUserProfile() {
    $.post(ctx + "sysuser/getUserProfile", {"userId": userId}, function (r) {
        if (r.code === 0) {
            ...
            $form.find("input[name='partyName']").val(user.partyName).attr("readonly", true);
            ...
            //$partyTree.jstree().open_all();
            //$partyTree.jstree('select_node', user.partyId, true);
        } else {
            $MB.n_danger(r.msg);
        }
    });
