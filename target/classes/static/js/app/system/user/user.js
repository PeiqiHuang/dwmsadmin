$(function () {
	...
	var $party = $userTableForm.find("select[name='partyId']");
    var settings = {
...
function deleteUsers() {
    ...
    $MB.confirm({
        ...
        $.post(ctx + 'sysuser/delete', {"ids": ids}, function (r) {
            if (r.code === 0) {
                $MB.n_success(r.msg);
                refresh();
            } else {
...
function exportUserExcel() {
    $.post(ctx + "sysuser/excel", $(".user-table-form").serialize(), function (r) {
        if (r.code === 0) {
...
function exportUserCsv() {
    $.post(ctx + "sysuser/csv", $(".user-table-form").serialize(), function (r) {
        if (r.code === 0) {
            window.location.href = "common/download?fileName=" + r.msg + "&delete=" + true;
        } else {
            $MB.n_warning(r.msg);
