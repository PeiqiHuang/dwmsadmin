...
function makeup() {
    ...
    var selected_length = selected.length;
    if (!selected_length) {
        $MB.n_warning('请勾选需要发起补考的考试！');
        return;
    }
    if (selected_length > 1) {
        $MB.n_warning('一次只能选择一个考试发起补考！');
        return;
    }
    var entityId = selected[0].examId;
	$MB.confirm({
		text: "未通过或未提交试卷人员将收到补考通知！确定发起补考？",
		confirmButtonText: "发起补考"
	}, function() {
		$.post(ctx + 'exam/makeup', { "examId": entityId }, function(r) {
			if (r.code === 0) {
				$MB.n_success(r.msg);
				refresh();
			} else {
...
function openQuUser() {
	...
	var entityId = selected[0].examId;
	var examName = selected[0].examName;
	var url = "examQuData?examId=" + entityId;
