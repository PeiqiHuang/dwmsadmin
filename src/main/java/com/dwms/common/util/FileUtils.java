...
public class FileUtils {
    ...
    private static String makeFileName(String filename) {
        return UUIDUtils.generateUUID() + "_" + filename;
    }
	...
	/**
     * 保存一个实体上传的一张图片(一个实只有多张图片，不清空目录)
     * 返回msg中存有图片保存路径
	 * 
	 * @author PeiqiHuang
	 * @date 2018年12月17日 上午9:49:07
	 * @param file
	 * @param subPath
	 * @return
	 */
	public static ResponseBo addImg(MultipartFile file, String subPath) {
	    if (null != file && !file.isEmpty()) {
	        String home = imageConfig.getUploadPath();
	        String path = imageConfig.getRoot() + subPath + "/";
	        try {
    ...
    public static void delImgs(List<String> imgPaths) {
        if (CollectionUtils.isEmpty(imgPaths)) return;
        File dir = new File(imageConfig.getUploadPath());
        if(dir.isDirectory()) {
            File file = null;
            for(String imgPath : imgPaths) {
                file = new File(dir.getAbsoluteFile() + "/" + imgPath);
                if (file.exists()) file.delete();
            }
        }
    }
